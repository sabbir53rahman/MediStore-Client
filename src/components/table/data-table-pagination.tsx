...
export function DataTablePagination<TData>({
    ...
    const totalPages = Math.ceil(paginationMeta.total / paginationMeta.limit);
    const currentPageSize = paginationMeta.limit;
    ...
    const handlePreviousClick = () => {
        if (currentPage > 1) {
            const newPageIndex = currentPage - 2; // Convert to 0-based index
    ...
    const handleNextClick = () => {
        if (currentPage < totalPages) {
            const newPageIndex = currentPage; // Convert to 0-based index
            table.setPageIndex(newPageIndex);
            updatePageParam(newPageIndex);
        }
    };
